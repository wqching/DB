...
class SquishReportHandler(xml.sax.handler.ContentHandler, xml.sax.handler.ErrorHandler):
    ...
    def startElement(self, name, attributes):
        if name == u"SquishReport":
            ...
            if not version:
                raise ReportVersionError("Missing Squish Report version; "
                        "please use squishrunner's xml2.2 report-generator.")
            version = version.split(".")
            if int(version[0]) < 2:
                raise ReportVersionError("Unsupported Squish Report version \"%s\"; "
                        "please use squishrunner's xml2.2 report-generator." % ".".join(version))
            elif int(version[0]) == 3:
                raise ReportVersionError("Unsupported Squish Report version \"%s\"; "
                        "please use squishxml3html.py instead." % ".".join(version))
            self.in_report = True
            return
        ...
        if name == u"test":
            ...
            self.message_file = attributes.get("file")
            self.message_line = attributes.get("line")
